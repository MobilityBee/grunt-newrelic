...
module.exports = function(grunt) {
	grunt.registerTask("newrelic", "generate a newrelic.js file", function(environment) {
		...
		try {
			...
			grunt.log.ok("Unable to load configuration file %s", config_path, e.toString());
		}

		try {
			newrelic_file = grunt.file.read("node_modules/newrelic/newrelic.js")
				.replace("app_name: ['My Application']", app_name)
				.replace("license_key: 'license key here'", license_key)
				.replace("level: 'info'", logging_level);
		}
		catch(e) {
			grunt.log.ok("Unable to load newrelic.js from %s", "node_modules/newrelic/newrelic.js", e.toString());
		}
		...
		try {
			...
			grunt.log.ok("Unable to write newrelic.js", e.toString());
		}
	});
};
